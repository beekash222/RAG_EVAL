...
if st.button("Evaluate"):
    if question and context and generated_output:
        ...
        st.write(f"**Semantic Similarity**: {metrics['Semantic Similarity']:.2f}")
        st.write("Semantic Similarity measures how semantically similar the generated output is to the reference text, using sentence embeddings. Range: 0-1. Higher scores indicate greater semantic similarity.")

        st.write(f"**Factual Consistency**: {'Yes' if metrics['Factual Consistency'] else 'No'}")
        st.write("Factual Consistency checks if the generated output aligns with the facts presented in the reference text based on a semantic similarity threshold.")

        st.write(f"**Question Relevance**: {'Yes' if metrics['Question Relevance'] else 'No'}")
        st.write("Question Relevance checks if the generated output is relevant to the input question based on a semantic similarity threshold.")


        st.write(f"**Context Relevance**: {'Yes' if metrics['Context Relevance'] else 'No'}")
        st.write("Context Relevance checks if the generated output aligns with the information provided in the context.")

        st.write(f"**Answer Relevance**: {'Yes' if metrics['Answer Relevance'] else 'No'}")
        st.write("Answer Relevance checks if the generated output is relevant to the input question.")


        st.write(f"**Toxicity**: {metrics['Toxicity']:.2f}")
        st.write("Toxicity measures the likelihood of the generated output being perceived as toxic or harmful. Range: 0-1. Lower scores indicate less toxicity.")
